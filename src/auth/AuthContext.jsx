...
export function AuthProvider({ children }) {
  ...
  async function checkUsername(username) { if (!supabase || !session?.user) return false; const { data, error } = await supabase.rpc('is_username_available', { candidate: username, current_user: session.user.id }); if (error) throw error; return Boolean(data); }
  async function updateUsername(username) { const normalized = username.trim().toLowerCase(); if (!/^[a-z0-9_]{3,24}$/.test(normalized)) throw new Error('Username harus 3–24 karakter: huruf kecil, angka, atau underscore.'); if (!supabase || !session?.user) throw new Error('Supabase belum dikonfigurasi.'); const available = await checkUsername(normalized); if (!available) throw new Error('Username sudah digunakan.'); const { data, error } = await supabase.from('profiles').update({ username: normalized, name: normalized, display_name: normalized, updated_at: new Date().toISOString() }).eq('id', session.user.id).select('*').single(); if (error) throw error; setProfile(data); return data; }
  async function signOut() { await supabase?.auth.signOut(); }
  return <AuthContext.Provider value={{ session, profile, loading, signIn, signUp, signInWithOtp, verifyOtp, signInWithGoogle, checkUsername, updateUsername, signOut, configured: isSupabaseConfigured }}>{children}</AuthContext.Provider>;
}
